import os
from evaluation_metrics_individual import nmse, ssim, haarpsi3d, vsi3d,psnr
import h5py
import numpy as np
...
def quantitively_evaluate_recons(datapath, reconpath, dataset="FastMRI"):
    ...
    for root, _, files in os.walk(reconpath):
        for file in files:
            ...
            for slice in range(num_slices):
                ...
                recon_slice = recon_slice / np.max(recon_slice)
                gt_slice = construct_gt(kspaces[slice], sensitivity_maps[slice], shift=True)
                gt_slice = gt_slice / np.max(gt_slice)
                if dataset == "FastMRI":
                ...
                haarpsi_score = haarpsi3d(gt_slice, recon_slice)
                vsi_score = vsi3d(gt_slice, recon_slice)
                psnr_score = psnr(gt_slice,recon_slice)

                subject_ssim.append(sim_score)
                subject_nmse.append(error_score)
                subject_haarpsi.append(haarpsi_score)
                subject_vsi.append(vsi_score)
                subject_psnr.append(psnr_score)

                print("SSIM", sim_score)
                print("HaarPSI", haarpsi_score)
                print("VSI", vsi_score)
                print("PSNR", psnr_score)

                plt.subplot(1, 2, 1)
                gt = np.abs(gt_data['target'][slice])
                ...
                plt.axis('off')
                # plt.colorbar()
                plt.title("Reconstruction" + ' SSIM: ' + str(np.round(sim_score, 2)))
                plt.show()
            ...
            # Uncomment the line below if you have PSNR scores available
            # print("PSNR mean subject:", np.round(np.mean(subject_psnr), 1))
            print("HaarPSI mean subject:", np.round(np.mean(subject_haarpsi), 3))
            print("VSI mean subject:", np.round(np.mean(subject_vsi), 3))

            ssim_scores += subject_ssim
            nrmse_scores += subject_nmse
            psnr_scores += subject_psnr
            haarpsi_scores += subject_haarpsi
            vsi_scores += subject_vsi
    ...
    print("HaarPSI mean all subjects:", np.mean(haarpsi_scores))
    print("VSI mean all subjects:", np.mean(vsi_scores))
    return ssim_scores
...
if __name__ == "__main__":
    ...
    data_path = '/scratch/helaajati/Esaote_Full_dataset/fold1/'
    # Path to reconstructions
    # recon_path = '/scratch/dmvandenberg/CIRIM_models_Esaote/Eerste_experiment/Spine test/default/2023-11-26_20-34-31/reconstructions/'
    recon_path = "/scratch/helaajati/Saved_models1/default/2024-01-08_19-33-50/reconstructions/"
    # get SSIM / NRMSE / PSNR
    ssim_list = quantitively_evaluate_recons(data_path, recon_path, dataset='Esaote')
